...
const createProduct = async (req, res, next) => {
  try {
    const { 
        name, description, price, sku, quantity, category, tags,
    ...
    if (!name || !price || !sku || !quantity) {
        res.status(400);
        throw new Error('Please fill in all required fields (Name, Price, SKU, Quantity)');
    }
    ...
    let imageUrl = req.body.image_url || req.body.imageUrl || ''; // Support camelCase or snake_case key
    if (req.file) {
    ...
    // Generate Slug
    const slug = name.toLowerCase().replace(/ /g, '-').replace(/[^\w-]+/g, '') + '-' + id.slice(0,8);

    // Insert into DB
    await db.query(
      `INSERT INTO products (
          id, name_en, name_ar, description_en, description_ar, price, sku, stock_quantity, category_id, image_url,
          tags, discount_type, discount_value,
          shipping_width, shipping_height, shipping_weight, shipping_cost,
      ...
      [
          ...
          price, sku, quantity, 
          categoryId, imageUrl,
          tagsJson, discount_type || 'no_discount', discount_value || 0,
          shipping_width, shipping_height, shipping_weight, shipping_cost,
          colorsJson, sizesJson, slug, req.body.is_featured ? 1 : 0
      ]
    );
    ...
    if (imageValues.length > 0) {
        ...
        // Update main product image_url for legacy support if needed (and if we found a primary one)
        // If imageUrl was empty initially, update to first image
    ...
    // Audit Log
    await logAdminAction(req.user.id, 'CREATE_PRODUCT', id, { name, sku });
...
const updateProduct = async (req, res, next) => {
  try {
    ...
    const categoryId = category || existing[0].category_id;
    ...
    await db.query(
      `UPDATE products SET 
          name_en=?, name_ar=?, description_en=?, description_ar=?, price=?, sku=?, stock_quantity=?, category_id=?,
          tags=?, discount_type=?, discount_value=?,
          shipping_width=?, shipping_height=?, shipping_weight=?, shipping_cost=?,
          colors=?, sizes=?, is_featured=?
       WHERE id=?`,
      [
          name, name, description, description, price, sku, quantity, categoryId,
          tagsJson, discount_type, discount_value,
          shipping_width, shipping_height, shipping_weight, shipping_cost,
          colorsJson, sizesJson, req.body.is_featured ? 1 : 0,
          productId
      ]
    );
    ...
    if (req.files && req.files.length > 0) {
        const imageValues = req.files.map((file, index) => {
            ...
            return [imgId, productId, imgUrl, isPrimary, index]; // You might want to adjust sort_order
        });
    ...
    // Audit Log
    await logAdminAction(req.user.id, 'UPDATE_PRODUCT', productId, { name, sku });
